...
bool CollisionModel3DImpl::collision(CollisionModel3D* other, int AccuracyDepth, 
										int MaxProcessingTime, float* other_transform){
  m_ColType=Models;
  ...
  unsigned EndTime,BeginTime = GetTickCount();
...
bool CollisionModel3DImpl::rayCollision(guVector _origin, 
                                        guVector _direction,
                                        bool closest,
                                        float segmin, 
                                        float segmax)
{
	...
	//--DCN: We're going to trick it for now; we'll make it better later

	float origin[3] = {_origin.x, _origin.y, _origin.z};
	float direction[3] = {_direction.x, _direction.y, _direction.z};
